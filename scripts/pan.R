...
setwd("/Users/oscarbruck/OneDrive - University of Helsinki/Tutkimus/Projekteja/Citations/")

# Create dirs
dir.create("combined/data", recursive = TRUE)
dir.create("combined/data_tmp", recursive = TRUE)
dir.create("combined/results/capital", recursive = TRUE)
dir.create("combined/results/continent", recursive = TRUE)
dir.create("combined/results/gender", recursive = TRUE)
dir.create("combined/results/map", recursive = TRUE)
dir.create("combined/results/publications", recursive = TRUE)
dir.create("combined/results/time", recursive = TRUE)

# Create dirs wo 2020 (COVID)
dir.create("combined_noncovid/data", recursive = TRUE)
dir.create("combined_noncovid/data_tmp", recursive = TRUE)
dir.create("combined_noncovid/results/capital", recursive = TRUE)
dir.create("combined_noncovid/results/continent", recursive = TRUE)
dir.create("combined_noncovid/results/gender", recursive = TRUE)
dir.create("combined_noncovid/results/map", recursive = TRUE)
dir.create("combined_noncovid/results/publications", recursive = TRUE)
dir.create("combined_noncovid/results/time", recursive = TRUE)
...
sum(df$nejm)/nrow(df)
sum(df$bmj)/nrow(df)
sum(df$lancet)/nrow(df)
sum(df$natmed)/nrow(df)
sum(df$jama)/nrow(df)
...
# Nejm
rm(dft)
for (i in list.files("nejm/data", full.names = TRUE, pattern = "xls")) {
  if (!exists("dft")) {
    dft <- readxl::read_xls(i)
  } else {
    dft = rbind(dft, readxl::read_xls(i))
  }
}
df_nejm <- dft %>%
  janitor::clean_names()
...
# Natmed
rm(dft)
for (i in list.files("natmed/data", full.names = TRUE, pattern = "xls")) {
  if (!exists("dft")) {
    dft <- readxl::read_xls(i)
  } else {
    dft = rbind(dft, readxl::read_xls(i))
...
# Jama
rm(dft)
for (i in list.files("jama/data", full.names = TRUE, pattern = "xls")) {
  if (!exists("dft")) {
    dft <- readxl::read_xls(i)
  } else {
    dft = rbind(dft, readxl::read_xls(i))
...
# Lancet
rm(dft)
for (i in list.files("lancet/data", full.names = TRUE, pattern = "xls")) {
  if (!exists("dft")) {
    dft <- readxl::read_xls(i)
  } else {
    dft = rbind(dft, readxl::read_xls(i))
...
# Bmj
rm(dft)
for (i in list.files("bmj/data", full.names = TRUE, pattern = "xls")) {
  if (!exists("dft")) {
    dft <- readxl::read_xls(i)
  } else {
    dft = rbind(dft, readxl::read_xls(i))
...
########################################## NORMALIZE CITATIONS BY JOURNAL ##########################################


# Preprocess
df_natmed$start_page <- as.character(df_natmed$start_page)
df_jama$start_page <- as.character(df_jama$start_page)
df_natmed$end_page <- as.character(df_natmed$end_page)
df_jama$end_page <- as.character(df_jama$end_page)
df_bmj$start_page <- as.character(df_bmj$start_page)
df_bmj$end_page <- as.character(df_bmj$end_page)

# Join
df <- full_join(full_join(full_join(full_join(df_nejm, df_natmed), df_jama), df_lancet), df_bmj)
...
# # Statistics
# cor.test(df$authors_tmp, df$times_cited_all_databases)
# plot(df$authors_tmp, df$times_cited_all_databases)
# cor.test(df$number_of_pages, df$times_cited_all_databases)
# plot(df$number_of_pages, df$times_cited_all_databases)
# # cor.test(df$issue, df$times_cited_all_databases)
# cor.test(nchar(df$article_title), df$times_cited_all_databases)
# plot(nchar(df$article_title), df$times_cited_all_databases)
# cor.test(df$reprint_addresses_tmp, df$times_cited_all_databases)
# plot(df$reprint_addresses_tmp, df$times_cited_all_databases)
# # df$reprint_addresses_tmp2 <- ifelse(is.na(df$reprint_addresses_tmp), NA,
# #                                    ifelse(df$reprint_addresses_tmp > 1, 1, 0))
# # wilcox.test(df$times_cited_all_databases ~ factor(df$reprint_addresses_tmp2))
# # plot(df$times_cited_all_databases ~ factor(df$reprint_addresses_tmp2))
# # cor.test(df$abstract_tmp, df$times_cited_all_databases)
# plot(df$abstract_tmp, df$times_cited_all_databases)
# df$abstract_tmp1 <- df$abstract_tmp/1000
# summary(lm(gsub("Inf", 0, log(times_cited_all_databases, 10))~abstract_tmp1, data=df))
# ggplot(df %>% dplyr::filter(abstract_tmp<2200), aes(abstract_tmp, log(times_cited_all_databases, 10))) + geom_point() + geom_smooth(method = "lm")
# hist(df$abstract_tmp, df$times_cited_all_databases)
# # cor.test(df$addresses_tmp, df$times_cited_all_databases)
# # head(df$reprint_addresses)
# # cor.test(df$author_per_address, df$times_cited_all_databases)
# plot(factor(df$open_access_designations_tmp), df$times_cited_all_databases)
# wilcox.test(df$times_cited_all_databases ~ factor(df$open_access_designations_tmp))
# df %>% group_by(open_access_designations_tmp) %>% summarise(mean(times_cited_all_databases))
...
# a$achiever3 <- as.factor(ifelse(a$n_articles_per_pop/a$n_articles  >= median(a$n_articles_per_pop/a$n_articles, na.rm=TRUE), 1, 0))
# shapiro.test(a$n_articles_per_pop)
...
writexl::write_xlsx(a_export, "combined_noncovid/data_tmp/table1.xlsx")


# ggplot(a, aes(x = n_articles_per_pop, y = log(times_cited_all_databases, 10), label = country.etc)) +
#   geom_point(aes(size=n_articles, fill=n_articles), shape = 21, color="black", alpha = 0.85) +
#   geom_abline(intercept = 0.375, slope = 0.09, color="black", size = 1, linetype = "solid") +
#   geom_label_repel(aes(color=achiever1)) +
#   scale_fill_distiller(palette = "RdBu", direction = -1, name="# Citations") +
#   scale_color_brewer(palette = "Set1", direction = -1, guide = "none") +
#   labs(y="# Citations (Log10)", x="# Publications/population") +
#   theme_bw() +
...
#   # scale_x_continuous(expand = c(0.01, 0.01), limits = c(0, ceiling(max(a$n_articles_per_pop, na.rm=TRUE)))) +
#   # scale_y_continuous(expand = c(0.00, 0.01), limits = c(ceiling(min(log(a$times_cited_all_databases, 10), na.rm=TRUE))-0.2, ceiling(max(log(a$times_cited_all_databases, 10), na.rm=TRUE))+0.2))
# ggsave(plot = g, filename = "combined_noncovid/results/publications/Correlation_publications_articles_per_pop.png", width = 5, height = 5, dpi = 300, units = "in")
# df %>% 
#   filter(`Saudi Arabia` == 1) %>%
#   filter(!str_detect(article_title, "Middle|MERS")) %>%
#   select(times_cited_all_databases)
# dplyr::mutate(a = median(times_cited_all_databases)) %>%
# select(a)
...
g <- ggplot(a, aes(y = log(n_articles, 10), x = log(times_cited_all_databases_sum, 10), label = paste0(country.etc, "\n", scales::percent(a$n_articles_diff_prop)))) +
  #geom_hline(yintercept = median(df$times_cited_all_databases_sum), size = 1, linetype = "dashed") +
  #geom_abline(intercept = 0.5, slope = 13.9, color="black", size = 1, linetype = "solid") +
  geom_smooth(method = "lm") +
  # geom_point(aes(size=n_articles_per_pop, color=achiever2)) +
  # geom_point(aes(fill=log(n_articles_per_pop, 10)), size = 14, shape = 21, color="black", alpha = 0.85) +
  ...
  theme(axis.text.x = element_text(size=12, colour = "black"),
        ...
        legend.position = "bottom")
# scale_x_continuous(expand = c(0.01, 0.01), limits = c(0, ceiling(max(log(a$n_articles, 10), na.rm=TRUE)))) +
# scale_y_continuous(expand = c(0.00, 0.01), limits = c(ceiling(min(a$times_cited_all_databases, na.rm=TRUE))-1, ceiling(max(a$times_cited_all_databases, na.rm=TRUE))+1))
ggsave(plot = g, filename = "combined_noncovid/results/publications/Correlation_publications_articles2.png", width = 8, height = 9, dpi = 300, units = "in")
...
g <- ggplot(a, aes(log(n_articles, 10), log(n_articles_per_pop, 10), label = country.etc)) +
  ...
  theme(axis.text.x = element_text(size=12, colour = "black"),
        ...
        legend.position = "bottom") +
  scale_size(range = c(1, 20), guide = "none")
# breaks = c(0, 1, 2, 3), limits = c(min(a$times_cited_all_databases), max(a$times_cited_all_databases)))
# scale_x_continuous(expand = c(0.01, 0.01), limits = c(0, ceiling(max(log(a$n_articles, 10), na.rm=TRUE)))) +
# scale_y_continuous(expand = c(0.00, 0.01), limits = c(ceiling(min(a$n_articles_per_pop, na.rm=TRUE))-1, ceiling(max(a$n_articles_per_pop, na.rm=TRUE))+1))
ggsave(plot = g, filename = "combined_noncovid/results/publications/Correlation_publications_articles.png", width = 8, height = 8, dpi = 300, units = "in")
...
# # Articles per city
# df_cities <- df[,c(which(colnames(df) == "reprint_addresses"), (which(colnames(df) == paste0("City_", length(grep("City_", colnames(df2))) )) +1) : (which(colnames(df) == "Country_1")-1))]
# df_cities <- reshape2::melt(df_cities, id.vars = "reprint_addresses", variable.name = "city", value.name = "n_articles") %>%
#   dplyr::filter(n_articles>0) %>%
#   dplyr::group_by(reprint_addresses) %>%
#   dplyr::summarise(n_articles = sum(n_articles)) %>%
#   ungroup()
...
# # Geolocate each city by their address
# df_long_city <- df_long_city %>%
#   mutate(coor = geocode(name))
# 
# # Save to avoid rerunning this
# saveRDS(df_long_city, "combined_noncovid/data_tmp/combined.rds")
# 
# 
# 
# # Call geolocated information
# df_long_city <- readRDS("combined_noncovid/data_tmp/combined.rds")
# 
# # Geolocation fails with abbreviations
# df_long_cityz <- df_long_cityz %>%
#   dplyr::mutate(
#     name = gsub("Hosp[ ,]", "Hospital ",
#                 gsub("Sch[ ,]", "School ",
#                      gsub("Univ[ ,]", "University ",
#                           gsub("Dept[ ,]", "Department ",
#                                gsub("Res[ ,]", "Research ",
#                                     gsub("Inst[ ,]", "Institute\\,",
#                                          gsub("Div[ ,]", "Division ",
#                                               gsub("Canc[ ,]", "Cancer ",
#                                                    gsub("NCI\\,", "National Cancer Institute,",
#                                                         gsub("Natl ", "National ",
#                                                              gsub("Mt ", "Mount ",
#                                                                   gsub("MSKCC ", "Memorial Sloan Kettering Cancer Center ",
#                                                                        gsub("Mem Sloan ", "Memorial ",
#                                                                             gsub("Cardiovasc ", "Cardiovascular ",
#                                                                                  gsub("Diabet ", "Diabetes ",
#                                                                                       gsub("Adv ", "Advanced ",
#                                                                                            gsub("Ist Sci ", "Institute of Science",
#                                                                                                 gsub(" Dis ", " Disease ", name)))))))))))))))))))
# 
# 
# df_long_cityz <- df_long_cityz %>%
#   dplyr::mutate(
#     name = gsub("Hlth[ ,]", "Heath ", 
#                 gsub("Ctr[ ,]", "Center ",
#                      gsub("Med Ctr", "Medical Center",
#                           gsub("Soc ", "Social ",
#                                gsub("Sci ", "Sci ",
#                                     gsub("Dis ", "Disease ",
#                                          gsub("Translat ", "Translational ",
#                                               gsub("Funct ", "Functional ",
#                                                    gsub("Appl ", "Applied ",
#                                                         gsub("Syst ", "Systems ",
#                                                              gsub("Dermatol ", "Dermatology ",
#                                                                   gsub("Clin ", "Clinical ",
#                                                                        gsub("MRC[ ,]", "Medical Research Council ",
#                                                                             gsub("NIH[ ,]", "National Institute of Health ",
#                                                                                  gsub("NIMH[ ,]", "National Institute of Mental Health ",
#                                                                                       gsub("NHGRI[ ,]", "National Human Genome Research Institute ", name)))))))))))))))))
# df_long_cityz <- df_long_cityz %>%
#   dplyr::mutate(
#     name = gsub("NEI[ ,]", "National Eye Institute ",
#                 gsub("NHLBI[ ,]", "National Heart, Lung, and Blood Institute ",
#                      gsub("NIAID[ ,]", "National Institute of Allergy and Infectious Diseases ",
#                           gsub("NICHD[ ,]", "National Institute of Child Health and Human Development ",
#                                gsub("NICD[ ,]", "National Institute of Communicable Diseases ",
#                                     gsub("NIAAA[ ,]", "National Institute on Alcohol Abuse and Alcoholism ",
#                                          gsub("UCL[ ,]", "University College London ",
#                                               gsub("WHO[ ,]", "World Health Organization ", 
#                                                    gsub("Off[ ,]", "Office ", 
#                                                         gsub("Invest[ ,]", "Investment ", 
#                                                              gsub("Prevent[ ,]", "Prevention ", 
#                                                                   gsub("Acad[ ,]", "Academy ", 
#                                                                        gsub("US ", "United States ", 
#                                                                             gsub("& Co Inc ", "Corporation ", 
#                                                                                  gsub("VHIO", "Vall d'Hebron Institute of Oncology",
#                                                                                       gsub("Org[ ,]", "Organization ", name)))))))))))))))))
# 
# 
...
# 
# # Save to avoid rerunning this
# saveRDS(df_long_city_final, "combined_noncovid/data_tmp/combined.rds")
df_long_city <- readRDS("combined_noncovid/data_tmp/combined.rds")
...
worldHDI <- ggplot(data = df_world, mapping = aes(x = long, y = lat, group = group)) + 
  ...
  plain
ggsave(plot = worldHDI, filename = "combined_noncovid/results/map/Map_country_number_of_citations_raw.png", width = 10, height = 8, dpi = 300, units = "in")
worldHDI <- ggplot(data = df_world, mapping = aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  geom_polygon(aes(fill = times_cited_all_databases1)) +
  scale_fill_distiller(palette ="Spectral", direction = -1, name="# Citations") + # or direction=1
  plain
ggsave(plot = worldHDI, filename = "combined_noncovid/results/map/Map_country_number_of_citations.png", width = 10, height = 8, dpi = 300, units = "in")
worldHDI <- ggplot(data = df_world, mapping = aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  geom_polygon(aes(fill = times_cited_all_databases2)) +
  scale_fill_distiller(palette ="Spectral", direction = -1, name="# Citations") + # or direction=1
  plain
ggsave(plot = worldHDI, filename = "combined_noncovid/results/map/Map_country_number_of_citations_min5articles.png", width = 10, height = 8, dpi = 300, units = "in")
...
worldHDI <- ggplot(data = df_world, mapping = aes(x = long, y = lat, group = group)) +
  ...
  plain
ggsave(plot = worldHDI, filename = "combined_noncovid/results/map/Map_country_number_of_articles_raw.png", width = 10, height = 8, dpi = 300, units = "in")
worldHDI <- ggplot(data = df_world, mapping = aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  geom_polygon(aes(fill = n_articles1)) +
  scale_fill_distiller(palette ="Spectral", direction = -1, name="# Publications") + # or direction=1
  plain
ggsave(plot = worldHDI, filename = "combined_noncovid/results/map/Map_country_number_of_articles.png", width = 10, height = 8, dpi = 300, units = "in")
worldHDI <- ggplot(data = df_world, mapping = aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  geom_polygon(aes(fill = n_articles2)) +
  scale_fill_distiller(palette ="Spectral", direction = -1, name="# Publications") + # or direction=1
  plain
ggsave(plot = worldHDI, filename = "combined_noncovid/results/map/Map_country_number_of_articles_min5articles.png", width = 10, height = 8, dpi = 300, units = "in")
...
# mapsimple = wrld_simpl
# # mapsimple = wrld_simpl
# mapsimple@data <- mapsimple@data %>%
#   dplyr::mutate(NAME = gsub("United States", "USA",
#                             gsub("United Kingdom", "UK",
#                                  gsub("Czechia", "Czech Republic",
#                                       gsub("Korea", "Korea South",
#                                            gsub("Russian Federation", "Russia",
#                                                 gsub("Serbia", "Serbia and Montenegro",
#                                                      gsub("Slovak Republic", "Slovakia",
#                                                           gsub("Syrian Arab Republic", "Syria",
...
#                                                                                                        gsub(",[[:print:]]*", "", NAME))))))))))))))))))
# mapsimple@data <- mapsimple@data %>%
#   dplyr::left_join(df_world %>% dplyr::select(NAME=region, starts_with("times_cited_all_databases"), starts_with("n_articles")) %>% distinct())
# 
# # Process
# ## NA
# mapsimple@data[11:ncol(mapsimple@data)][is.na(mapsimple@data[11:ncol(mapsimple@data)])] <- 0
# ## Keep only if n of articles > 5
# mapsimple@data$n_articles1 <- ifelse(mapsimple@data$n_articles<5, round(rnorm(mean=3, n = 1, sd = 1.5), 0), mapsimple@data$n_articles)
# # mapsimple@data$n_articles1 <- ifelse(mapsimple@data$n_articles1==0, 0.01, mapsimple@data$n_articles1)
# 
# ## Map
# sfno <- st_as_sf(mapsimple)
# sfproj <- st_transform(sfno, crs = 4087)   # 4087, 3785 = world, 4326 = Europe, 23038, 3857 = Africa
# # sfproj <- spTransform(mapsimple, CRS("+init=epsg:23038"))
# st_crs(sfproj)
# 
# # Construct a cartogram
# cartogram <- cartogram_cont(sfproj, weight = "n_articles1", itermax = 5, threshold = 0.05)
# # tm_shape(cartogram) + tm_polygons("times_cited_all_databases4", style = "jenks") +
# #   tm_layout(frame = FALSE, legend.position = c("left", "bottom"))
# 
# # cartogram1 <- cartogram_cont(sfproj, "POP2005", itermax = 5)
# # tm_shape(cartogram1) + tm_polygons("POP2005", style = "jenks") +
# #   tm_layout(frame = FALSE, legend.position = c("left", "bottom"))
# 
# # Plot
# ggplot(data = cartogram) +
#   geom_sf(aes(fill = n_articles1)) +
#   scale_fill_distiller(palette ="Spectral", direction = -1, name="# Citations") + # or direction=1
#   plain
# 
# 
...
ggsave(plot = g, "combined_noncovid/results/continent/Barplot_continent_n_articles.png", width = 5, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_continent %>% dplyr::filter(!is.na(continent)), aes(x = reorder(continent, -times_cited_all_databases_sum), y = times_cited_all_databases_sum, fill = continent)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/continent/Barplot_continent_n_citations.png", width = 5, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_continent %>% dplyr::filter(!is.na(continent)), aes(x = reorder(continent, -times_cited_all_databases_median), y = times_cited_all_databases_median, fill = continent)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/continent/Barplot_continent_n_citations_per_articles.png", width = 5, height = 4, units = "in", dpi = 300)
...
# Plot
ggsave(plot = worldHDI, filename = "combined_noncovid/results/map/map_city_citation.png", units = "in", dpi = 300, width = 20, height = 12)

# Plot
ggsave(plot = worldHDI2, filename = "combined_noncovid/results/map/map_city_citation_publications.png", units = "in", dpi = 300, width = 20, height = 12)
...
ggsave(plot = worldHDI1, filename = "combined_noncovid/results/map/map_city_USA_citations.png", units = "in", dpi = 300, width = 10, height = 8)
ggsave(plot = worldHDI2, filename = "combined_noncovid/results/map/map_city_USA_publications.png", units = "in", dpi = 300, width = 10, height = 7)
...
ggsave(plot = worldHDI, filename = "combined_noncovid/results/map/map_city_Europe.png", units = "in", dpi = 300, width = 10, height = 10)
ggsave(plot = worldHDI2, filename = "combined_noncovid/results/map/map_city_Europe_publications.png", units = "in", dpi = 300, width = 10, height = 10)
...
g <- ggplot(df_world_city4 %>% dplyr::filter(!is.na(capital)) %>% dplyr::filter(country.etc %in% df_world_city4_countries), aes(x = capital, y = log(times_cited_all_databases, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/capital/Boxplot_capital_n_citations.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_world_city4 %>% dplyr::filter(!is.na(capital)) %>% dplyr::filter(country.etc %in% df_world_city4_countries), aes(x = capital, y = log(n_articles, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/capital/Boxplot_capital_n_articles.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_world_city4 %>% dplyr::filter(!is.na(capital)) %>% dplyr::filter(country.etc %in% df_world_city4_countries), aes(x = capital, y = cit_per_art)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/capital/Boxplot_capital_n_citations_per_articles.png", width = 4, height = 4, units = "in", dpi = 300)
...
for (i in unique(df_world[!is.na(df_world$continent),]$continent)) {
  ...
  g <- ggplot(df_world_city4 %>% dplyr::filter(country.etc %in% tmp$region) %>% dplyr::filter(!is.na(capital)) %>% dplyr::filter(country.etc %in% df_world_city4_countries), aes(x = capital, y = log(times_cited_all_databases, 10))) +
    ...
    theme(axis.text.x = element_text(size=14, colour = "black"),
          ...
          legend.key = element_rect(color="black"),
          legend.position = "NULL")
  ggsave(plot = g, paste0("combined_noncovid/results/capital/Boxplot_capital_n_citations_", i, ".png"), width = 4, height = 4, units = "in", dpi = 300)
  ...
  g <- ggplot(df_world_city4 %>% dplyr::filter(country.etc %in% tmp$region) %>% dplyr::filter(!is.na(capital)) %>% dplyr::filter(country.etc %in% df_world_city4_countries), aes(x = capital, y = log(n_articles, 10))) +
    ...
    theme(axis.text.x = element_text(size=14, colour = "black"),
          ...
          legend.key = element_rect(color="black"),
          legend.position = "NULL")
  ggsave(plot = g, paste0("combined_noncovid/results/capital/Boxplot_capital_n_articles_", i, ".png"), width = 4, height = 4, units = "in", dpi = 300)
  ...
  g <- ggplot(df_world_city4 %>% dplyr::filter(country.etc %in% tmp$region) %>% dplyr::filter(!is.na(capital)) %>% dplyr::filter(country.etc %in% df_world_city4_countries), aes(x = capital, y = cit_per_art)) +
    ...
    theme(axis.text.x = element_text(size=14, colour = "black"),
          ...
          legend.key = element_rect(color="black"),
          legend.position = "NULL")
  ggsave(plot = g, paste0("combined_noncovid/results/capital/Boxplot_capital_n_citations_per_articles_", i, ".png"), width = 4, height = 4, units = "in", dpi = 300)
...
for (i in 1:nrow(df_authors)) {
  ...
  if(!is.na(df_authors[i,1])) {
    ## First author
    # df_authors1[i,2] <- df_authors[i,(df[i,]$authors_tmp+1)]
    # df_authors1[i,3] <- df_authors[i,(df[i,]$authors_tmp+2)]
    # NAindex <- which(is.na(df_authors[i,]))
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_first_gender)), aes(x = author_first_gender, y = log(times_cited_all_databases, 10))) +
  ...
  stat_compare_means(method = "wilcox.test",
                     label = "p.signif",
                     # label.y = 0.9*max(log(df$times_cited_all_databases, 10), na.rm=TRUE),
                     label.x = 1.45,
                     size = 6) +
  theme_bw() +
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_citations_first_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_second_gender)), aes(x = author_second_gender, y = log(times_cited_all_databases, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_citations_second_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_second_last_gender)), aes(x = author_second_last_gender, y = log(times_cited_all_databases, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_citations_second_last_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_last_gender)), aes(x = author_last_gender, y = log(times_cited_all_databases, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_citations_last_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_first_gender)), aes(x = author_first_gender, y = log(number_of_pages, 10))) +
  ...
  stat_compare_means(method = "wilcox.test",
                     label = "p.signif",
                     # label.y = 0.9*max(log(df$number_of_pages, 10), na.rm=TRUE),
                     label.x = 1.45,
                     size = 6) +
  theme_bw() +
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_pages_first_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_second_gender)), aes(x = author_second_gender, y = log(number_of_pages, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_pages_second_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_second_last_gender)), aes(x = author_second_last_gender, y = log(number_of_pages, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_pages_second_last_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_last_gender)), aes(x = author_last_gender, y = log(number_of_pages, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_pages_last_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_first_gender)), aes(x = author_first_gender, y = log(authors_tmp, 10))) +
  ...
  stat_compare_means(method = "wilcox.test",
                     label = "p.signif",
                     # label.y = 0.9*max(log(df$authors_tmp, 10), na.rm=TRUE),
                     label.x = 1.45,
                     size = 6) +
  theme_bw() +
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_authors_first_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_second_gender)), aes(x = author_second_gender, y = log(authors_tmp, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_authors_second_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_second_last_gender)), aes(x = author_second_last_gender, y = log(authors_tmp, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_authors_second_last_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df %>% dplyr::filter(!is.na(author_last_gender)), aes(x = author_last_gender, y = log(authors_tmp, 10))) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Boxplot_n_authors_last_author_gender.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_sr %>% dplyr::filter(!is.na(author_first_gender)), aes(x = author_first_gender, y = n_articles, fill = author_first_gender)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Barplot_n_articles_first_author_gender.png", width = 5, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_sr %>% dplyr::filter(!is.na(author_second_gender)), aes(x = author_second_gender, y = n_articles, fill = author_second_gender)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Barplot_n_articles_second_author_gender.png", width = 5, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_sr %>% dplyr::filter(!is.na(author_second_last_gender)), aes(x = author_second_last_gender, y = n_articles, fill = author_second_last_gender)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Barplot_n_articles_second_last_author_gender.png", width = 5, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_sr %>% dplyr::filter(!is.na(author_last_gender)), aes(x = author_last_gender, y = n_articles, fill = author_last_gender)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/gender/Barplot_n_articles_last_author_gender.png", width = 5, height = 4, units = "in", dpi = 300)
...
g <- ggplot(df_gender_sr, aes(x = author_first_gender, y=Freq, fill=author_last_gender)) +
  ...
  theme(axis.text.x = element_text(size=12, colour = "black"),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "bottom")
ggsave(plot = g, "combined_noncovid/results/gender/Barplot_first_last_author_gender.png", width = 3, height = 4, units = "in", dpi = 300)
  ...
  for (j in c("number_of_pages_cat", "authors_tmp_cat")) {
    ...
    g <- ggplot(df %>% dplyr::filter(!is.na(tmp)), aes(x = tmp1, y = log(times_cited_all_databases, 10), fill=factor(tmp))) +
      ...
      theme(axis.text.x = element_text(size=14, colour = "black"),
            ...
            legend.text = element_text(size=14, colour = "black"),
            legend.position = "bottom")
    ggsave(plot = g, paste0("combined_noncovid/results/gender/Boxplot_citations_", j, "_", i, ".png"), width = 4, height = 4, units = "in", dpi = 300)
...
p <- ggballoonplot(df_plot1, y = "Author", x = "Country",
  ...
  scale_size(breaks = c(0.1, 0.5, 1),
             # breaks = c(min(df_plot1$Prop_scale_abs), median(df_plot1$Prop_scale_abs), max(df_plot1$Prop_scale_abs)),
             # labels=c("Min", "Med", "Max"),
             labels=c(0.1, 0.5, 1),
             # labels = round(c(min(df_plot1$Prop_scale_abs), median(df_plot1$Prop_scale_abs), max(df_plot1$Prop_scale_abs)), 1),
             range = c(1, 10)) +
  # scale_fill_gradient2(low = "blue", mid = "white", high = "red",
  #                      # midpoint = 0,
  #                      midpoint = median(df_plot1$Prop_scale),
  #                      breaks = c(-1, round(median(df_plot1$Prop_scale), 1), floor(max(df_plot1$Prop_scale)*10)/10),
  #                      labels = c(-1, round(median(df_plot1$Prop_scale), 1), floor(max(df_plot1$Prop_scale)*10)/10),
  #                      # limits = c(-1, 1),
  #                      na.value = "gray") +
...
p
ggsave(plot = p, filename = "combined_noncovid/results/gender/Balloonplot_gender_country.png", width = 8, height = 3.5, units = "in", dpi = 300)




# ### Mann-Whitney U test
# multiple_t_tests_p_value <- lapply(df[countries$country.etc], function(x) wilcox.test(x ~ df$author_first_gender, na.rm=TRUE, exact=FALSE))
...
#   sum_male = unlist(lapply(df[countries$country.etc], function(x) sum(x[df$author_first_gender=="Male"], na.rm=TRUE))),
#   sum_female = unlist(lapply(df[countries$country.etc], function(x) sum(x[df$author_first_gender=="Female"], na.rm=TRUE)))
# ) %>%
#   ### Rownames to column
#   rownames_to_column() %>%
...
#   dplyr::mutate(
#     Author = "First",
#     Prop = sum_female / (sum_male+sum_female) * 100)
# 
# 
# ### Second author
# multiple_t_tests_p_value <- lapply(df[countries$country.etc], function(x) wilcox.test(x ~ df$author_second_gender, na.rm=TRUE, exact=FALSE))
# ### P-values can be extracted from the result object
# pvalue <- data.frame(p.value = sapply(multiple_t_tests_p_value, getElement, name = "p.value"))
# ### Create a matrix and dataframe of the p-values
# pvalue_df_second <- pvalue %>% data.frame() %>% mutate(
#   #### Add the p values to a new dataframe
#   p_adjusted = p.adjust(p = as.matrix(pvalue), method = "BH"),
#   #### Add also the t values, 95%CI to the same dataframe
#   sum_male = unlist(lapply(df[countries$country.etc], function(x) sum(x[df$author_second_gender=="Male"], na.rm=TRUE))),
#   sum_female = unlist(lapply(df[countries$country.etc], function(x) sum(x[df$author_second_gender=="Female"], na.rm=TRUE)))
# ) %>%
#   ### Rownames to column
#   rownames_to_column() %>%
#   rename(Country = rowname,
...
# 
# ### Second last author
# multiple_t_tests_p_value <- lapply(df[countries$country.etc], function(x) wilcox.test(x ~ df$author_second_last_gender, na.rm=TRUE, exact=FALSE))
# ### P-values can be extracted from the result object
# pvalue <- data.frame(p.value = sapply(multiple_t_tests_p_value, getElement, name = "p.value"))
# ### Create a matrix and dataframe of the p-values
# pvalue_df_second_last <- pvalue %>% data.frame() %>% mutate(
#   #### Add the p values to a new dataframe
#   p_adjusted = p.adjust(p = as.matrix(pvalue), method = "BH"),
#   #### Add also the t values, 95%CI to the same dataframe
#   sum_male = unlist(lapply(df[countries$country.etc], function(x) sum(x[df$author_second_last_gender=="Male"], na.rm=TRUE))),
#   sum_female = unlist(lapply(df[countries$country.etc], function(x) sum(x[df$author_second_last_gender=="Female"], na.rm=TRUE)))
# ) %>%
#   ### Rownames to column
#   rownames_to_column() %>%
#   rename(Country = rowname,
...
# 
# ### Last author
# multiple_t_tests_p_value <- lapply(df[countries$country.etc], function(x) wilcox.test(x ~ df$author_last_gender, na.rm=TRUE, exact=FALSE))
# ### P-values can be extracted from the result object
# pvalue <- data.frame(p.value = sapply(multiple_t_tests_p_value, getElement, name = "p.value"))
# ### Create a matrix and dataframe of the p-values
# pvalue_df_last <- pvalue %>% data.frame() %>% mutate(
#   #### Add the p values to a new dataframe
#   p_adjusted = p.adjust(p = as.matrix(pvalue), method = "BH"),
#   #### Add also the t values, 95%CI to the same dataframe
#   sum_male = unlist(lapply(df[countries$country.etc], function(x) sum(x[df$author_last_gender=="Male"], na.rm=TRUE))),
#   sum_female = unlist(lapply(df[countries$country.etc], function(x) sum(x[df$author_last_gender=="Female"], na.rm=TRUE)))
# ) %>%
#   ### Rownames to column
#   rownames_to_column() %>%
#   rename(Country = rowname,
...
# 
# ### Combine
# df_country_gender <- full_join(full_join(full_join(pvalue_df_first, pvalue_df_second), pvalue_df_second_last), pvalue_df_last)
# ### Process
# df_country_gender1 <- df_country_gender %>%
#   dplyr::mutate(Author = factor(Author, levels = c("First", "Second", "Second last", "Last")),
#                 Prop_scale = Prop/50 - 1,
#                 pvalue_scale = -log(pvalue, 10),
#                 p_adjusted_scale = -log(p_adjusted, 10))
# 
# # Balloonplot
# p <- ggballoonplot(df_country_gender1, y = "Author", x = "Country",
#                    fill = "Prop_scale",
#                    size = "pvalue_scale",
#                    size.range = c(1, 10),
#                    ggtheme = theme_bw()) +
#   scale_size(breaks = c(0, 1, 2, 3), range = c(2, 10), limits = c(1, max(df_country_gender1$pvalue_scale))) +
#   scale_fill_gradient2(low = "blue", mid = "white", high = "red",
#                        midpoint = 0,
...
ggsave(plot = top20, filename = "combined_noncovid/results/gender/keywords_top20_first_author.png", width = 8, height = 8, units = "in", dpi = 300)
...
top20 <- ggplot(pvalue_df_first_low10_first, aes(fill = F_to_M_first, ymax = ymax, ymin = ymin, xmax = 100, xmin = 80)) +
  ...
  theme(legend.title = element_text(colour = "black", size = 16, face = "bold"), 
        ...
        panel.border = element_blank(),
        panel.background = element_blank())
ggsave(plot = top20, filename = "combined_noncovid/results/gender/keywords_low20_first_author.png", width = 8, height = 8, units = "in", dpi = 300)
...
top20 <- ggplot(pvalue_df_first_top10_last, aes(fill = F_to_M_last, ymax = ymax, ymin = ymin, xmax = 100, xmin = 80)) +
  ...
  theme(legend.title = element_text(colour = "black", size = 16, face = "bold"), 
        ...
        panel.border = element_blank(),
        panel.background = element_blank())
ggsave(plot = top20, filename = "combined_noncovid/results/gender/keywords_top20_last.png", width = 8, height = 8, units = "in", dpi = 300)
...
top20 <- ggplot(pvalue_df_first_low10_last, aes(fill = F_to_M_last, ymax = ymax, ymin = ymin, xmax = 100, xmin = 80)) +
  ...
  theme(legend.title = element_text(colour = "black", size = 16, face = "bold"), 
        ...
        panel.border = element_blank(),
        panel.background = element_blank())
ggsave(plot = top20, filename = "combined_noncovid/results/gender/keywords_low20_last.png", width = 8, height = 8, units = "in", dpi = 300)
...
plt <- ggplot() +
  ...
  geom_col(aes(x = 3, y = prop*2, fill = value), 
           data = pvalue_df_first_top10_last2[pvalue_df_first_top10_last2$variable!="F_to_M_last",], color = "black") +
  # geom_text(aes(label = paste(Keyword, round(prop, 1),"%"), x= 3.5, y = 194.7797-Pos), hjust=1, vjust=-1, data = pvalue_df_first_top10_last2[pvalue_df_first_top10_last2$variable=="F_to_M_last",], size = 3)+
...
g
ggsave(plot = g, filename = "combined_noncovid/results/gender/keywords_first20_combined.png", width = 10, height = 8, units = "in", dpi = 300)
...
plt <- ggplot() +
  ...
  geom_col(aes(x = 3, y = prop*2, fill = value), 
           data = pvalue_df_first_low10_first2[pvalue_df_first_low10_first2$variable!="F_to_M_last",], color = "black") +
  # geom_text(aes(label = paste(Keyword, round(prop, 1),"%"), x= 3.5, y = 194.7797-Pos), hjust=1, vjust=-1, data = pvalue_df_first_low10_first2[pvalue_df_first_low10_first2$variable=="F_to_M_last",], size = 3)+
  ...
  theme(legend.title = element_text(colour = "black", size = 16, face = "bold"), 
        ...
        panel.background = element_blank())
g <- plt + coord_polar(theta = "y") 
ggsave(plot = g, filename = "combined_noncovid/results/gender/keywords_low20_combined.png", width = 10, height = 8, units = "in", dpi = 300)
...
plt <- ggplot() +
  ...
  theme(legend.title = element_blank(),
        ...
        panel.background = element_blank())
g <- plt + coord_polar(theta = "y") 
ggsave(plot = g, filename = "combined_noncovid/results/gender/keywords_combined_legend.png", width = 4, height = 4, units = "in", dpi = 300)
...
g <- ggplot(pvalue_df_first1_spiral,
  ...
  theme(legend.title = element_text(colour = "black", size = 16, face = "bold"), 
        ...
        panel.border = element_blank(),
        panel.background = element_blank())
ggsave(plot = g, filename = "combined_noncovid/results/gender/keywords_all_first_author.png", width = 6, height = 6, units = "in", dpi = 300)
...
g <- ggplot(pvalue_df_first1_spiral,
  ...
  theme(legend.title = element_text(colour = "black", size = 16, face = "bold"), 
        ...
        panel.border = element_blank(),
        panel.background = element_blank())
ggsave(plot = g, filename = "combined_noncovid/results/gender/keywords_all_last_author.png", width = 6, height = 6, units = "in", dpi = 300)
...
# 
# # Define dataset
# df_kw4 <- cbind(df %>% dplyr::select(journal_abbreviation), df_kw3[w]) %>%
#   dplyr::rename(Journal = journal_abbreviation) %>%
#   dplyr::mutate(Journal = ifelse(Journal=="JAMA-J AM MED ASSOC", "JAMA",
#                                  ifelse(Journal=="NAT MED", "NATMED",
#                                         ifelse(str_detect(Journal, "BMJ"), "BMJ",
#                                                ifelse(Journal=="NEW ENGL J MED", "NEJM", "LANCET")))))
# 
# # As character
# df_kw4[2:ncol(df_kw4)] <- sapply(df_kw4[2:ncol(df_kw4)], function(x) as.character(x))
# 
# # Shuffle rows
# df_kw4 = df_kw4[sample(1:nrow(df_kw4)), ]
# 
# # MCA
# res.mca <- MCA(df_kw4[,2:ncol(df_kw4)],
#                graph=FALSE)
# 
# # All
# grp <- as.factor(df_kw4[, "Journal"])
# g <- fviz_mca_ind(res.mca, pointsize = 0.2, alpha.ind = 0.8, habillage = grp, palette="Set1", geom="point")
# ggsave(plot = g, filename = "combined_noncovid/results/publications/MCA_all.png", width = 6, height = 6, units = "in", dpi = 300)
# 
# # By journals
# df_kw4$Journal <- as.factor(df_kw4$Journal)
# set1_palette = c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00")
# for (i in unique(df_kw4$Journal)) {
#   # Define dataset
#   df_kw4 <- cbind(df %>% dplyr::select(journal_abbreviation), df_kw3[w]) %>%
#     dplyr::rename(Journal = journal_abbreviation) %>%
#     dplyr::mutate(Journal = ifelse(Journal=="JAMA-J AM MED ASSOC", "JAMA",
...
#   df_kw4$Journal <- as.factor(df_kw4$Journal)
#   # Sort
#   df_kw4$Journal1 <- ifelse(df_kw4$Journal == i, 1, 0)
#   df_kw4$Journal1 <- as.factor(df_kw4$Journal1)
#   df_kw4 <- df_kw4[order(df_kw4$Journal1),]
#   
#   res.mca <- MCA(df_kw4[,2:ncol(df_kw4)],
#                  graph=FALSE)
#   grp <- as.factor(df_kw4[, "Journal1"])
#   col = set1_palette[which(levels(df_kw4$Journal)==i)]
#   g <- fviz_mca_ind(res.mca, pointsize = 0.2, habillage = grp, palette=c("white", col), geom="point")
#   ggsave(plot = g, filename = paste0("combined_noncovid/results/publications/MCA_", i, ".png"), width = 6, height = 6, units = "in", dpi = 300)
# }
# 
# ### UMAP
# library(uwot)
# umap1 <- umap(as.matrix(df_kw3[w]), n_neighbors = 5, metric = "hamming")
# colnames(umap1)[1:2] = c("UMAP1", "UMAP2")
# umap1 <- cbind(df_kw3[,1:5], umap1)
# umap1 <- cbind(umap1, df %>% dplyr::select(journal_abbreviation))
# umap1 <- umap1 %>%
#   dplyr::rename(Journal = journal_abbreviation) %>%
#   dplyr::mutate(Journal = ifelse(Journal=="JAMA-J AM MED ASSOC", "JAMA",
#                                  ifelse(Journal=="NAT MED", "NATMED",
...
# # Plot
# g <- umap1 %>%
#   arrange(author_first_gender) %>%
#   ggplot(aes(UMAP1, UMAP2)) +
#   geom_point(aes(color = Journal), size = 0.2, alpha = 0.8) +
...
# ggsave(plot = g, filename = "combined_noncovid/results/publications/Umap_all.png", width = 4, height = 4, units = "in", dpi = 300)
# 
# # Plot
# for (i in unique(umap1$Journal)) {
#   set1_palette = c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00")
#   col = set1_palette[which(levels(umap1$Journal)==i)]
#   umap1$col = ifelse(umap1$Journal == i, col, "white")
#   
#   g <- umap1 %>%
#     arrange(author_first_gender) %>%
#     # filter(Journal=="NEJM") %>%
#     filter(Journal %in% !!i) %>%
#     # filter(Journal=="LANCET") %>%
#     # filter(Journal=="NATMED") %>%
#     # filter(Journal=="JAMA") %>%
#     ggplot(aes(UMAP1, UMAP2)) +
#     geom_point(aes(color = Journal), size = 0.2, alpha = 0.8) +
#     # scale_color_brewer(palette = "Set1") +
...
#           legend.position = "bottom",
#           # legend.key.size = unit(1, 'cm'),
#           legend.title = element_text(size=10, face="bold", colour = "black", vjust = 0.85),
#           legend.text = element_text(size=10, colour = "black"),
#           axis.text.y = element_text(size=0, colour = "black"),
#           axis.title=element_text(size=10, face="bold", colour = "black"))
#   ggsave(plot = g, filename = paste0("combined_noncovid/results/publications/Umap_", i, ".png"), width = 4, height = 4, units = "in", dpi = 300)
# }
# 
# 
# # MCA
# library(factoextra)
# library(FactoMineR)
# umap1 <- umap(as.matrix(df_kw3[w]), n_neighbors = 15)
# colnames(umap1)[1:2] = c("UMAP1", "UMAP2")
# umap1 <- cbind(df_kw3[,1:5], umap1)
# umap1 <- cbind(umap1, df %>% dplyr::select(journal_abbreviation))
# umap1 <- umap1 %>%
...
#                                         ifelse(str_detect(Journal, "BMJ"), "BMJ",
#                                                ifelse(Journal=="NEW ENGL J MED", "NEJM", "LANCET")))))
# # df_kw4 <- reshape2::melt(df_kw4, id.vars="Journal") %>%
# #   filter(value>0) %>%
# #   dplyr::select(-value)
# # df_kw4 <- reshape2::dcast(df_kw4, variable~Journal, fun.aggregate = seq_along)
# # df_kw4[1:5,1:5]
# m1 <- col(df_kw4[,2:ncol(df_kw4)][-1]) * df_kw4[,2:ncol(df_kw4)][-1]
# i1 <- m1 != 0
# df_kw4[,2:ncol(df_kw4)][-1][i1] <- rep(colnames(m1), each = nrow(m1))[i1]
# res.mca <- MCA(as.matrix(df_kw4[,2:ncol(df_kw4)]), graph = FALSE)
...
summary(model)
model <- lm(times_cited_all_databases_log ~ author_first_gender + number_of_pages + China + authors_tmp, data = df)
summary(model)

# 
# 
# # Extract covariates and variable to predict for regression model
# train <- df %>%
#   dplyr::mutate_at(.vars = c("author_first_gender", "author_second_gender", "author_second_last_gender", "author_last_gender"), function(x) ifelse(is.na(x), 0.5, ifelse(x=="male", 1, 0))) %>%
#   dplyr::select(times_cited_all_databases, authors_tmp, ends_with("gender"), number_of_pages, countries$country.etc)
#   # dplyr::select(times_cited_all_databases, authors_tmp, ends_with("gender"), number_of_pages)
# train$times_cited_all_databases_log <- log(train$times_cited_all_databases, 10)
# train$times_cited_all_databases_log <- ifelse(is.infinite(train$times_cited_all_databases_log), NA, train$times_cited_all_databases_log)
# 
# 
# cor.test(train$times_cited_all_databases, train$authors_tmp, method = "spearman")
# plot(train$times_cited_all_databases_log, log(train$authors_tmp, 10), method = "spearman")
# cor.test(train$times_cited_all_databases, train$author_first_gender, method = "spearman")
# cor.test(train$times_cited_all_databases_log, train$author_first_gender, method = "spearman")
# 
# 
# # Stepwise Regression
# # library(MASS)
# fit <- lm(times_cited_all_databases_log ~ authors_tmp, data=train)
# # fit <- lm(times_cited_all_databases ~ ., data=train)
...
# ## Create model
# train$model1 <- NULL
# for (coef in 1:nrow(t$coefficients)) {
#   if (rownames(t$coefficients)[coef] == "(Intercept)") {
#     train$model1 = t$coefficients[coef]
#   }
#   else {
#     train$model1 = train$model1 + t$coefficients[coef]*train[rownames(t$coefficients)[coef]]
#   }
# }
# train$model1 <- unlist(train$model1)
# # fit <- lm(times_cited_all_databases_log ~ log(model1, 10), data=train)
# fit <- lm(times_cited_all_databases_log ~ log(model1, 10), data=train)
# summary(fit)$adj.r.squared
# cor.test(train$times_cited_all_databases_log, train$model1, method = "spearman")
# cor.test(train$times_cited_all_databases_log, train$model1, method = "pearson")
# 
# ggplot(train, aes(log(model1, 10), log(times_cited_all_databases, 10))) +
#   geom_point() +
...
for (i in unique(duplicate_city_names$name)) {
  # i = unique(duplicate_city_names$name)[1]
  # i = "Adelaide"
  # print(i)
  # print(colSums(df_city_country[df_city1[i]==1,duplicate_city_names[duplicate_city_names$name==i,]$country.etc]))
...
for (i in dupl_cities1[is.na(dupl_cities1$dupl),]$City) {
  for (j in colnames(dupl_cities1)[str_detect(colnames(dupl_cities1), "Country_to_fix")]) {
    ...
    tmp2 = ifelse(tmp == "UK",
                  # paste0(i, sprintf("[^;]*"), "UK|", i, sprintf("[^;]*"), "England|", i, sprintf("[^;]*"), "Scotland|", i, sprintf("[^;]*"), "Wales"),
                  # paste0(i, sprintf("[^;]*"), tmp))
...
# df_city1_sr_1$achiever2 <- log(df_city1_sr_1$times_cited_all_databases, 10)/log(df_city1_sr_1$n, 10)
# df_city1_sr_1$achiever3 <- as.factor(ifelse(df_city1_sr_1$achiever2>1, 1, 0))
...
g <- ggplot(df_city1_sr_1, aes(y = log(n, 10), x = log(times_cited_all_databases_sum, 10),
  ...
  # scale_fill_brewer(palette = "Set1", direction = -1, name="# Publication excess", guide = "none") +
  ...
  theme(axis.text.x = element_text(size=12, colour = "black"),
        ...
        legend.spacing.y = unit(0.1, 'cm'),
        legend.position = "bottom")
# scale_y_continuous(expand = c(0.01, 0.01), limits = c(0.8, max(log(df_city1_sr_1$n, 10), na.rm=TRUE)+0.1)) +
# scale_x_continuous(expand = c(0.01, 0.01), limits = c(0.8, max(log(df_city1_sr_1$times_cited_all_databases_sum, 10), na.rm=TRUE)+0.1))
ggsave(plot = g, filename = "combined_noncovid/results/publications/Correlation_publications_articles_city.png", width = 8, height = 9, dpi = 300, units = "in")
...
g <- ggplot(pvalue_df_journals %>% dplyr::filter(Journal=="NEJM"), aes(x = reorder(Country, -Prop), y = Prop, fill = Prop)) +
  ...
  # ylim(0, max(pvalue_df_journals %>% dplyr::filter(Journal=="NEJM") %>% dplyr::select(Prop) %>% arrange(desc(Prop)) %>% slice(1)+10) ) +
  # scale_y_continuous(limits = c(0,100), expand = c(0, 0)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/journals/Barplot_publications_nejm.png", width = 10, height = 4, units = "in", dpi = 300)
...
g <- ggplot(pvalue_df_journals %>% dplyr::filter(Journal=="LANCET"), aes(x = reorder(Country, -Prop), y = Prop, fill = Prop)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/journals/Barplot_publications_lancet.png", width = 10, height = 4, units = "in", dpi = 300)
...
g <- ggplot(pvalue_df_journals %>% dplyr::filter(Journal=="NATMED"), aes(x = reorder(Country, -Prop), y = Prop, fill = Prop)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/journals/Barplot_publications_natmed.png", width = 10, height = 4, units = "in", dpi = 300)
...
g <- ggplot(pvalue_df_journals %>% dplyr::filter(Journal=="BMJ"), aes(x = reorder(Country, -Prop), y = Prop, fill = Prop)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, filename = "combined_noncovid/results/journals/Barplot_publications_bmj.png", width = 10, height = 4, units = "in", dpi = 300)
...
g <- ggplot(pvalue_df_journals %>% dplyr::filter(Journal=="JAMA"), aes(x = reorder(Country, -Prop), y = Prop, fill = Prop)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/journals/Barplot_publications_jama.png", width = 10, height = 4, units = "in", dpi = 300)
...
g <- ggplot(pvalue_df_country2, aes(x = Journal, y = Prop, fill=Journal)) +
  ...
  # geom_text(aes(label = sig, x=2.5, 1), vjust = -0.1, colour = "black") +
  # scale_fill_distiller(palette = "Spectral", name="Country") +
  theme_bw() +
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        axis.text.y = element_text(size=14, colour = "black"),
...
ggsave(plot = g, "combined_noncovid/results/journals/Barplot_publications_all.png", width = 10, height = 4, units = "in", dpi = 300)
...
ggsave(plot = g, "combined_noncovid/results/journals/Gender_publications_all.png", width = 6, height = 2.75, units = "in", dpi = 300)
...
# save.image(file='combined_noncovid/data_tmp/Session.RData')
# load('combined_noncovid/data_tmp/Session.RData')
...
for (i in c("author_first_gender", "author_last_gender", "number_of_pages_cat", "authors_tmp_cat")) {
                                                                     ...
                                                                     # gsub("gender", "sex",
                                                                          gsub("number of pages cat", "# Pages",
                                                                               gsub("authors tmp cat", "# Authors",
                                                                                    gsub("_", " ", i))))))) +
    # scale_size_continuous(name = "# Publications",
    #                       breaks = c(300, 500, 700),
    #                       limits = c(0, 1000),
    #                       range = c(1, 10)
    # ) +
    # breaks = seq(from = min(round(df_plot$n/1000, 1)*1000), to = max(round(df_plot$n/1000, 1)*1000), by = max(round(df_plot$n/1000, 1)*1000)-min(round(df_plot$n/1000, 1)*1000) ),
    # labels = seq(from = min(round(df_plot$n/1000, 1)*1000), to = max(round(df_plot$n/1000, 1)*1000), by = max(round(df_plot$n/1000, 1)*1000)-min(round(df_plot$n/1000, 1)*1000) )) +
    # breaks = c(min(round(df_plot$n/1000, 1)*1000), (max(round(df_plot$n/1000, 1)*1000)+min(round(df_plot$n/1000, 1)*1000))/2, max(round(df_plot$n/1000, 1)*1000))) +
    guides(color = FALSE,
           fill = guide_legend(override.aes = list(size=8), order=1)) +
           # size = guide_legend(ncol = 3, order=2)) +
    theme_bw() +
    theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
          axis.text.y = element_text(size=14, colour = "black"),
          axis.title.y = element_text(size=14, face="bold", colour = "black"),
  ...
  g
  ggsave(plot = g, filename = paste0("combined_noncovid/results/time/", i, "_citations_by_publication_year.png"), width = 5.5, height = 5, units = "in", dpi = 300)
  ...
  g <- ggplot(data = df_plot, aes(x = publication_year, y = n, group = tmp)) +
    ...
    theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
          ...
          legend.direction = "horizontal")
  g
  ggsave(plot = g, filename = paste0("combined_noncovid/results/time/", i, "_publications_by_publication_year.png"), width = 5.5, height = 5, units = "in", dpi = 300)
...
for (i in c("author_first_gender", "author_last_gender")) {
  ...
  df_plot <- df %>%
    dplyr::filter(!is.na(tmp)) %>%
    # dplyr::select(tmp, publication_year, one_of(a[a$n_articles > 19,]$country.etc), times_cited_all_databases) %>%
  ...
  g <- ggplot(data = df_plot, aes(x = publication_year, y = n, group = tmp)) +
    geom_line(size = 1.5) +
    # geom_smooth(method="lm", aes(col = tmp), size = 2, se=FALSE) +
    xlab("Publication year") +
  ...
  ggsave(plot = g, filename = paste0("combined_noncovid/results/time/", i, "_citations_by_publication_year_by_country.png"), width = 20, height = 15, units = "in", dpi = 300)
}
...
# Adjust p values
## number_of_pages
# pairwise.test1 = df_plot1 %>%
#   mutate(publication_year1=as.numeric(as.character(publication_year))-2009) %>%
#   cor_test(publication_year1, med, method = "spearman") %>%
#   mutate(p = ifelse(p < 0.001, "p<0.001",
#                     ifelse(p < 0.01, "p<0.01",
#                            ifelse(p < 0.05, "p<0.05", round(p, 2)))))
# 
# ## authors_tmp
# pairwise.test2 = df_plot2 %>%
#   mutate(publication_year1=as.numeric(as.character(publication_year))-2009) %>%
#   cor_test(publication_year1, med, method = "spearman") %>%
#   mutate(p = ifelse(p < 0.001, "p<0.001",
#                     ifelse(p < 0.01, "p<0.01",
...
g <- ggplot(data = df_plot, aes(x = publication_year, y = med)) +
  ...
  # geom_errorbar(aes(ymin = iqr25, ymax = iqr75, group=variable, color=variable), position = "dodge", width = 0.25) +
  xlab("Publication year") +
  ylab("Number") +
  # geom_point(aes(fill=variable), size=4, col="black", shape=21, position=position_dodge(width = .25)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        ...
        legend.direction = "horizontal")
g
ggsave(plot = g, filename = "combined_noncovid/results/time/Corplot_authors_and_pages_by_publication_year.png", width = 5.5, height = 5, units = "in", dpi = 300)
...
g <- ggplot(data = df_plot, aes(x = publication_year, y = times_cited_all_databases, group = Country)) +
  ...
  # override.aes = list(size=15), order=1)) +
  theme_bw() +
...
ggsave(plot = g, filename = "combined_noncovid/results/time/Country_citation_year.png", width = 17, height = 12, units = "in", dpi = 300)
...
g <- ggplot(data = df_plot, aes(x = publication_year, y = n, group = Country)) +
  ...
  theme(axis.text.x = element_text(size=20, colour = "black", angle = 45, hjust = 1),
        ...
        # legend.direction = "horizontal",
        strip.text.x = element_text(size = 15))
ggsave(plot = g, filename = "combined_noncovid/results/time/Country_publication_year.png", width = 17, height = 12, units = "in", dpi = 300)
...
g <- ggplot(data = df_plot, aes(x = publication_year, y = n, group = City)) +
  ...
  theme(axis.text.x = element_text(size=20, colour = "black", angle = 45, hjust = 1),
        ...
        # legend.direction = "horizontal",
        strip.text.x = element_text(size = 15))
ggsave(plot = g, filename = "combined_noncovid/results/time/City_publication_year.png", width = 17, height = 12, units = "in", dpi = 300)
...
g <- ggplot(data = df_plot, aes(x = publication_year, y = times_cited_all_databases, group = City)) +
  ...
  theme(axis.text.x = element_text(size=20, colour = "black", angle = 45, hjust = 1),
        ...
        # legend.direction = "horizontal",
        strip.text.x = element_text(size = 15))
ggsave(plot = g, filename = "combined_noncovid/results/time/City_citation_year.png", width = 17, height = 12, units = "in", dpi = 300)
...
#   }
# }
# write_tsv(as.data.frame(zz1), "combined_noncovid/data_tmp/doi.tsv")
#### Find citing countries from Web of Science and read the resulting data
citing <- readxl::read_xlsx("combined_noncovid/data_tmp/doi_post.xlsx")
...
g <- ggplot(zz, aes(x = reorder(citing_country, -self_citations_prop), y = self_citations_prop, fill = citing_country)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/publications/Barplot_country_citation1.png", width = 10, height = 5, units = "in", dpi = 300)
...
g <- ggplot(zz, aes(x = reorder(citing_country, -self_citation_index), y = self_citation_index, fill = citing_country)) +
  ...
  theme(axis.text.x = element_text(size=14, colour = "black", angle = 45, hjust = 1),
        ...
        legend.key = element_rect(color="black"),
        legend.position = "NULL")
ggsave(plot = g, "combined_noncovid/results/publications/Barplot_country_citation2.png", width = 10, height = 5, units = "in", dpi = 300)
...
# a1$country.etc = factor(a1$country.etc, levels = c(unique(a1[order(a1$citations_freq, decreasing = TRUE),]$country.etc)))
...
# a1$citing_country = factor(a1$citing_country, levels = levels(a1$country.etc))
a1$citations_freq1 <- log(a1$citations_freq, 10)
# summary(a1$citations_freq_prop)
...
g <- ggballoonplot(a1, x = "country.etc", y = "citing_country",
              ...
              # size.range = c(2, 10),
              ggtheme = theme_bw()) +
  # scale_size(breaks = c(0, 1, 2, 3), range = c(2, 10), limits = c(1, max(a1$citations_freq))) +
  scale_size(range = c(1, 10), labels=c(10,20,30), breaks=c(10,20,30)) +
  # scale_size(range = c(1, 8), labels=c(1,3,6), breaks=c(1,3,6)) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red",
                       # labels=c(2,3),
                       # breaks=c(2,3),
                       midpoint = 3.5,
...
ggsave(plot = g, filename = "combined_noncovid/results/publications/Balloonplot_citations_country.png", width = 9, height = 7, units = "in", dpi = 300)
...
ggsave(plot = ggd1, filename = "combined_noncovid/results/publications/Balloonplot_citations_country_clusters1.png", width = 2, height = 5, units = "in", dpi = 300)





# devtools::install_github("muschellij2/rscopus", force = T)
# 
# library(rscopus)
# rscopus::set_api_key(api_key = '815361a7c5db8ab915a8da8480c7ee2d')
# # token is from Scopus dev
# # hdr = inst_token_header("890ac4aca3872b38976355f324cdc2b9")
...
# susa <- df %>% dplyr::rename(Journal = journal_abbreviation) %>%
#       dplyr::mutate(Journal = ifelse(Journal=="JAMA-J AM MED ASSOC", "JAMA",
#                                      ifelse(Journal=="NAT MED", "NATMED",
#                                             ifelse(str_detect(Journal, "BMJ"), "BMJ",
#                                                    ifelse(Journal=="NEW ENGL J MED", "NEJM", "LANCET"))))) %>%
#   group_by(Journal, publication_year) %>%
#   summarise(n = n())
# 
# ggplot(data = susa, aes(x=publication_year, y=n, color=Journal, group=Journal)) + geom_point() + geom_line(size = 1.5)
